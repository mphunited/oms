...
export function BillToContactFields({ control, register }: {
    ...
    <div className="space-y-3">
      ...
      {fields.map((field, idx) => (
        <div key={field.id} className="grid grid-cols-5 gap-2 rounded-md border p-3">
          <div className="col-span-2 space-y-1"><Label className="text-xs text-muted-foreground">Name</Label><Input placeholder="Full name" {...register(`bill_to_contacts.${idx}.name`)} /></div>
          <div className="col-span-3 space-y-1"><Label className="text-xs text-muted-foreground">Email</Label>
            <div className="flex gap-1.5">
              <Input type="email" placeholder="email@company.com" {...register(`bill_to_contacts.${idx}.email`)} />
