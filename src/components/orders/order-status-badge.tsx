...
const statusColors: Record<string, string> = {
  ...
  "Wash & Return Stage":            "bg-purple-50 text-purple-700 border-purple-200",
...
export function OrderStatusBadge({ status, color }: { status: OrderStatus | string; color?: string }) {
  ...
  return (
    <span className={`inline-block rounded-md px-2.5 py-0.5 text-xs font-medium border text-center ${classes}`}>
      {status}
    </span>
  )
}
