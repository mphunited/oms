...
import { useEditDrumForm } from '@/lib/recycling/use-edit-drum-form'
...
export function EditDrumForm({ id }: { id: string }) {
  const {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 pb-10">
      ...
      {/* Customer & Processing Facility */}
      <Section title="Customer & Processing Facility">
        <Row>
      ...
      <Section title="Ship From">
        {(['name','street','city','state','zip'] as const).map(k => (
          <Field key={k} label={k.charAt(0).toUpperCase() + k.slice(1)}>
            <Input value={form.ship_from[k]} onChange={e => setShipFrom(k, e.target.value)} />
          </Field>
        ))}
      </Section>
      ...
      <Section title="Bill To">
        {(['name','street','city','state','zip'] as const).map(k => (
          <Field key={k} label={k.charAt(0).toUpperCase() + k.slice(1)}>
            <Input value={form.bill_to[k]} onChange={e => setBillTo(k, e.target.value)} />
          </Field>
        ))}
      </Section>
      ...
      {/* Customer Confirmation Contacts */}
      <Section title="Customer Confirmation Contacts">
        {form.customer_contacts.map((c, i) => (
          <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-end">
            <Input placeholder="Name" value={c.name} onChange={e => updateCustContact(i, { name: e.target.value })} />
            <Input placeholder="Email" type="email" value={c.email} onChange={e => updateCustContact(i, { email: e.target.value })} />
            <button onClick={() => removeCustContact(i)} className="text-destructive hover:opacity-70">
              <Trash2 className="h-4 w-4" />
            </button>
          </div>
        ))}
        <Button type="button" variant="outline" size="sm" onClick={addCustContact}>
          <Plus className="h-3.5 w-3.5 mr-1" />Add Contact
        </Button>
      </Section>
