...
export function NewDrumForm() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 pb-10">
      ...
      <Section title="Ship From / Pickup Location">
      ...
      <Section title="Customer Confirmation Contacts">
        {form.customer_contacts.map((c, i) => (
          <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-end">
            <Input placeholder="Name" value={c.name} onChange={e => updateCustomerContact(i, { name: e.target.value })} />
            <Input placeholder="Email" type="email" value={c.email} onChange={e => updateCustomerContact(i, { email: e.target.value })} />
            <button onClick={() => removeCustomerContact(i)} className="text-destructive hover:opacity-70">
              <Trash2 className="h-4 w-4" />
            </button>
          </div>
        ))}
        <Button type="button" variant="outline" size="sm" onClick={addCustomerContact}>
      ...
      <Button onClick={submit} disabled={submitting} className="w-full bg-[#00205B] hover:bg-[#B88A44] text-white">
